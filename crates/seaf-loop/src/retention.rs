...
fn inventory_runs(
    ...
    for name in names {
        ...
        let pending_migration = if eligible_status(run.status)
            && !has_live_candidate
            && !migration_evidence
        {
            crate::migration::pending_migration_source_is_protected(root, name_text).map_err(
                |error| {
                    RetentionError::context(
                        format!("could not authenticate pending migration state for {name_text}"),
                        error,
                    )
                },
            )?
        ...
        let protection = if !eligible_status(run.status) || has_live_candidate {
            Protection::Active
        } else if migration_evidence || pending_migration {
            Protection::MigrationEvidence
