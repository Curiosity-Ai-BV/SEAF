...
or make any artifact durable outside the short-lived workflow run. Separately
authorized M2-05 published the workflow-verified `v0.1.0` assets without adding
write, OIDC, or attestation authority to this workflow.
...
## Published v0.1.0 Prerelease

The separately authorized
[`v0.1.0` prerelease](https://github.com/Curiosity-Ai-BV/SEAF/releases/tag/v0.1.0)
is immutable and targets exact commit
`f4d7c28d27c345a8b0d7f6cc48c8c833b48f248a`. Its public assets are:
...
After downloading the assets, verify their automatic GitHub release
attestations with:

```bash
gh release verify-asset v0.1.0 seaf-v0.1.0-aarch64-apple-darwin.tar.gz --repo Curiosity-Ai-BV/SEAF
gh release verify-asset v0.1.0 seaf-v0.1.0-x86_64-unknown-linux-gnu.tar.gz --repo Curiosity-Ai-BV/SEAF
gh release verify-asset v0.1.0 SHA256SUMS --repo Curiosity-Ai-BV/SEAF
```

These attestations are generated by immutable GitHub Release publication, not
by the read-only build workflow. The local installed-package smoke executed the
macOS arm64 binary; Linux execution evidence is the successful Ubuntu job in
the initial [tag workflow run](https://github.com/Curiosity-Ai-BV/SEAF/actions/runs/29318734239).
