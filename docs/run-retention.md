...
paired backup evidence. An eligible source in a pending whole-run migration is
also protected when its deterministic canonical intent authenticates the exact
source bytes. A staged sibling must additionally authenticate its ownership,
projected inventory, migrated authority, and result binding. Malformed or
mismatched controls for that source fail purge closed; an unrelated dot-prefixed
migration-like sibling remains excluded but does not protect an ordinary run.
Purge does not manage unrelated non-dot files, non-UTF-8 names, symlinks, broad
permissions, hard links, or malformed runs; it fails closed on those entries.
Every managed regular file must have exactly one link before intent publication.
A file hard-linked outside the run therefore makes dry-run and apply fail
without creating control state or unlinking either name.
...
is not `cleaned`, when it contains migration-result evidence, when it is the
authenticated source of a pending migration publication, or when its existing
mutation lock is busy. `eval_passed` and `promoted` are protected final authority:
their candidates must remain active and the supported workflow does not provide
a cleanup transition. Every other run status is active for retention purposes
